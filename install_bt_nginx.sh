...
web_camouflage(){
    ...
    if [[ -d "/home/wwwroot" ]];then
        echo -e "${OK} ${GreenBG} Web目录已存在， ${Font}"
        cd /home/wwwroot
    else
    mkdir -p /home/wwwroot && cd /home/wwwroot
    fi
...
domain_check(){
    read -p "请输入你的域名信息(eg:www.yesei.com):" domain
...
ExecReload=/www/server/nginxx/sbin/nginx -s reload
...
tls_type(){
    if [[ -f "$nginx_dir" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]];then
